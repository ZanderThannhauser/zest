...
const unsigned zebu_shifts[309][38] = {
	[1][2] = 2,
	[1][3] = 3,
	[1][4] = 4,
	[1][5] = 5,
...
const unsigned zebu_reduces[316][38] = {
	[2][7] = 1,
	[2][9] = 1,
	[2][12] = 1,
	[2][13] = 1,
...
void print_zebu_complex_command(struct link* links, enum prefix p, const char* name, struct zebu_complex_command* ptree)
{
	...
	if (ptree->rinfd)
		print_token_leaf(new ?: links, p_not_last_child, "rinfd", ptree->rinfd);
	else
		print_empty_leaf(new ?: links, p_not_last_child, "token", "rinfd");
	if (ptree->routfd)
		print_token_leaf(new ?: links, p_not_last_child, "routfd", ptree->routfd);
	else
		print_empty_leaf(new ?: links, p_not_last_child, "token", "routfd");
...
void print_zebu_multiplicative_expression(struct link* links, enum prefix p, const char* name, struct zebu_multiplicative_expression* ptree)
{
	...
	if (ptree->rem)
		print_token_leaf(new ?: links, p_not_last_child, "rem", ptree->rem);
	else
		print_empty_leaf(new ?: links, p_not_last_child, "token", "rem");
	if (ptree->right)
...
void print_zebu_prefix_expression(struct link* links, enum prefix p, const char* name, struct zebu_prefix_expression* ptree)
{
	...
	if (ptree->bitneg)
		print_token_leaf(new ?: links, p_not_last_child, "bitneg", ptree->bitneg);
	else
		print_empty_leaf(new ?: links, p_not_last_child, "token", "bitneg");
	if (ptree->file)
	...
	if (ptree->logneg)
		print_token_leaf(new ?: links, p_not_last_child, "logneg", ptree->logneg);
	else
		print_empty_leaf(new ?: links, p_not_last_child, "token", "logneg");
	if (ptree->numneg)
		print_token_leaf(new ?: links, p_not_last_child, "numneg", ptree->numneg);
	else
		print_empty_leaf(new ?: links, p_not_last_child, "token", "numneg");
	if (ptree->shell)
...
void free_zebu_complex_command(struct zebu_complex_command* ptree)
{
	if (ptree && !--ptree->refcount)
	{
		...
		free_zebu_token(ptree->rinfd);
		free_zebu_token(ptree->routfd);
		for (unsigned i = 0, n = ptree->subcommands.n; i < n; i++)
...
void free_zebu_prefix_expression(struct zebu_prefix_expression* ptree)
{
	if (ptree && !--ptree->refcount)
	{
		free_zebu_token(ptree->bitneg);
		free_zebu_expression(ptree->file);
		free_zebu_primary_expression(ptree->inner);
		free_zebu_token(ptree->logneg);
		free_zebu_token(ptree->numneg);
...
if (trie->rem) { free_zebu_token(value->rem); value->rem = inc_zebu_token(trie->rem); }
...
if (token->line < value->startline) value->startline = token->line;
if (value->endline < token->line) value->endline = token->line;
free_zebu_token(value->rem), value->rem = inc_zebu_token(token);
free_zebu_token(token);
}
{
struct zebu_multiplicative_expression* subgrammar = data.data[--yacc.n, --data.n];
...
if (trie->startline < value->startline) value->startline = trie->startline;
if (value->endline < trie->endline) value->endline = trie->endline;
if (trie->bitneg) { free_zebu_token(value->bitneg); value->bitneg = inc_zebu_token(trie->bitneg); }
...
if (trie->logneg) { free_zebu_token(value->logneg); value->logneg = inc_zebu_token(trie->logneg); }
if (trie->numneg) { free_zebu_token(value->numneg); value->numneg = inc_zebu_token(trie->numneg); }
...
if (token->line < value->startline) value->startline = token->line;
if (value->endline < token->line) value->endline = token->line;
free_zebu_token(value->logneg), value->logneg = inc_zebu_token(token);
free_zebu_token(token);
}
		d = value, g = 13;
		break;
...
if (token->line < value->startline) value->startline = token->line;
if (value->endline < token->line) value->endline = token->line;
free_zebu_token(value->numneg), value->numneg = inc_zebu_token(token);
free_zebu_token(token);
}
		d = value, g = 13;
		break;
...
if (token->line < value->startline) value->startline = token->line;
if (value->endline < token->line) value->endline = token->line;
free_zebu_token(value->bitneg), value->bitneg = inc_zebu_token(token);
free_zebu_token(token);
}
		d = value, g = 13;
		break;
